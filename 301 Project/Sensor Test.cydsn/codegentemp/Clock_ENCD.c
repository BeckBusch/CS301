...
/*******************************************************************************
* Function Name: Clock_ENCD_Start
********************************************************************************
*
* Summary:
...
void Clock_ENCD_Start(void) 
{
    ...
    Clock_ENCD_CLKEN |= Clock_ENCD_CLKEN_MASK;
	Clock_ENCD_CLKSTBY |= Clock_ENCD_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENCD_Stop
********************************************************************************
*
* Summary:
...
void Clock_ENCD_Stop(void) 
{
    ...
    Clock_ENCD_CLKEN &= (uint8)(~Clock_ENCD_CLKEN_MASK);
	Clock_ENCD_CLKSTBY &= (uint8)(~Clock_ENCD_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_ENCD_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_ENCD_CLKEN & Clock_ENCD_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_ENCD_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_ENCD_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_ENCD__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_ENCD_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_ENCD_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_ENCD_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_ENCD_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_ENCD_GetSourceRegister();
    uint16 oldDivider = Clock_ENCD_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_ENCD_CLKEN & Clock_ENCD_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_ENCD_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_ENCD_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_ENCD_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_ENCD_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_ENCD_MOD_SRC |= modeBitMask & (uint8)Clock_ENCD_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENCD_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_ENCD_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_ENCD_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_ENCD_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_ENCD_MOD_SRC & (uint8)(Clock_ENCD_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_ENCD_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_ENCD_GetDividerRegister();
    uint8 oldSrc = Clock_ENCD_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_ENCD_MOD_SRC |= CYCLK_SSS;
        Clock_ENCD_MOD_SRC =
            (Clock_ENCD_MOD_SRC & (uint8)(~Clock_ENCD_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_ENCD_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_ENCD_MOD_SRC & Clock_ENCD_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_ENCD_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_ENCD_PHASE = clkPhase & Clock_ENCD_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ENCD_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_ENCD_PHASE & Clock_ENCD_PHASE_MASK;
}
